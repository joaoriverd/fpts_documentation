...
\begin{proof}
%	Assume a level-$n$ busy period $[t_s,t_f]$ for task-set $\mathpzc{T}$, and also assume that task $\tau_{n+1}$ is scheduled in such a way that a job $k$ of $\tau_{n+1}$ executes within the interval $[t_1,t_s)$ as described in Lemma 1. We now show that, under this condition, the schedule in the interval $[t_s,t_f)$ remains the same after introducing $\tau_{n+1}$.
...
%	we want to show that the \textit{best-case response time} $BR_i$ of $\tau_i$ is equal to $BC_i$ when $\tau_i$ is non-preemptive, i.e. when $\theta_i = \pi_1$. 
